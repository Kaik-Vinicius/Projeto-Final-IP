...
def main():
    ...
    #Iniciando a logica para a criação das oportinidades de gols atráves do cronometro
    minuto_atual = 0
    intervalo_minuto_ms = 300        
    ultimo_tick_relogio = 0
    minuto_proximo_ataque = 0

    #Adicionando a logica de divisão por blocos para o surgimento de oportunidades de gols de acordo com o nivel
    oportunidades_totais = 0
    oportunidades_restantes = 0
    tamanho_bloco = 0                # Guarda quantos minutos dura cada bloco de tempo
    bloco_atual = 0

    #Contadores dos coletaveis para o placar
    chuteiras_coletadas = 0
    ...
    while rodando:
        ...
        if estado == "espera":
            if (minuto_atual >= 90):
                print("Fim do jogo!") # Aqui é apenas um print com o fim do jogo,ainda falta definir o que acontece quando o jogo acaba
                estado = "menu"
            #Implementando a lógica para quando acabar as oportunidades o crônometro passar mais rápido
            if (oportunidades_restantes > 0):
                intervalo_minuto_rápido_ms = intervalo_minuto_ms
            else:
                intervalo_minuto_rápido_ms = 200
            if (tempo_atual - ultimo_tick_relogio >= intervalo_minuto_rápido_ms):
                minuto_atual += 1
        ...
        if estado == "jogando":
            ...
            if distancia < 250:
                zagueiro1.perseguir_neymar(neymar)
            else:
                zagueiro1.idle()
        ...
        if estado == "menu":
            ...
            barra_superior = pygame.Surface((LARGURA_TELA, 50), pygame.SRCALPHA)
            barra_superior.fill((0, 0, 0, 180)) 
            tela.blit(barra_superior, (0, 0))
            
            txt_chuteiras = fonte_pequena.render(f" 👟 Chuteiras: {chuteiras_coletadas}", True, (255, 255, 255))
            txt_estrelas = fonte_pequena.render(f" ⭐ Estrelas: {estrelas_coletadas}", True, (255, 255, 255))
            txt_chances = fonte_pequena.render(f" Chances Restantes: {oportunidades_restantes}", True, (255, 215, 0))
            
            tela.blit(txt_chuteiras, (20, 15))
            tela.blit(txt_estrelas, (200, 15))
            tela.blit(txt_chances, (LARGURA_TELA - 260, 15))
            
            # Barra inferior translúcida do Cronômetro
            barra_inferior = pygame.Surface((LARGURA_TELA, 60), pygame.SRCALPHA)
            barra_inferior.fill((0, 0, 0, 180))
            tela.blit(barra_inferior, (0, ALTURA_TELA - 60))
            
            txt_relogio = fonte_jogo.render(f" ⏱️ {minuto_atual}' MIN", True, (255, 255, 255))
            tela.blit(txt_relogio, (20, ALTURA_TELA - 45))
            #Adicionando a mensagem de espera de oportunidade de ataque, que pisca a cada meio segundo
            if (tempo_atual // 500) % 2 == 0:
                if oportunidades_restantes <= 0:
                    txt_aguardando = fonte_pequena.render("OPORTUNIDADES ESGOTADAS... AGUARDANDO APITO FINAL!", True, (255, 100, 100))
                else:
                    txt_aguardando = fonte_pequena.render("AGUARDANDO OPORTUNIDADE DE ATAQUE...", True, (255, 255, 0))
                
                pos_x_texto = (LARGURA_TELA // 2) - (txt_aguardando.get_width() // 2)
                tela.blit(txt_aguardando, (pos_x_texto, ALTURA_TELA - 40))
